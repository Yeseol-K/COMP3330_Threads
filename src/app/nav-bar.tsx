...
export default function NavBar({ className }: { className?: string }) {
  return (
    <nav className={twMerge("flex h-full justify-between items-center", className)}>
      ...
      <Link href="/me" className="hover:bg-neutral-100 dark:hover:bg-neutral-900 flex justify-center items-center h-14 w-20 rounded-md transition-all fill-none stroke-neutral-600">
        <ProfileIcon />
      </Link>
    </nav>
  );
